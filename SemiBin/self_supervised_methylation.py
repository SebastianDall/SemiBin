...
import sys
from .meth_model import Self_encoding_dual_stream, loss_function
from .semi_supervised_model import feature_Dataset
...
def train_self(logger, datapaths, data_splits, is_combined=True,
    ...
    n_kmer_features = len(features_data["kmer"])
    n_motif_features = len(features_data["motif"])
    n_motif_present_features = len(features_data["motif_present"])
    n_depth_features = len(features_data["depth"])
    ...
    if not is_combined:
        total_features = n_kmer_features + n_motif_features
    else:
        total_features = n_kmer_features + n_motif_features + n_depth_features

    torch.set_num_threads(num_process)

    logger.info('Training model...')
    
    model = Self_encoding_dual_stream(
        total_features = total_features,
        n_kmer_features = n_kmer_features,
        n_motif_features = n_motif_features,
        n_motif_present_features=n_motif_present_features,
    )
        ...
        for data_index, (datapath, data_split_path) in enumerate(zip(datapaths, data_splits)):
            ...
            if not is_combined:
                if not features_data["motif"]:
                    ...
                    train_data_motif = data[features_data['motif']].values
                    train_data_motif_present = data[features_data['motif_present']].values

                    train_data_split_motif = data_split[features_data['motif']].values
                    train_data_split_motif_present = data_split[features_data['motif_present']].values
                    
                    train_data = np.concatenate(
                        (
                            train_data_kmer,
                            train_data_motif,
                            train_data_motif_present
                        ), axis = 1
                    )
                    
                    train_data_split = np.concatenate(
                        (
                            train_data_split_kmer,
                            train_data_split_motif,
                            train_data_split_motif_present
                        ), axis = 1
                    )
            ...
            if epoch % 5 == 0:
                ...
                logger.info(f"Feature weights - Kmer: {weights['kmer']:.3f}, Methylation: {weights['methylation']:.3f}")
        scheduler.step()
